            ...
            case 15
                if int16(crc(A,1,144)) == get_crc(A)                       
                    ...
                    if i > 1
                        ...
                        delta_yaw   = gz(i,1)*dt;
                        uint_g = RZ(RY(RX(uint_g,delta_roll),delta_pitch),delta_yaw);
                    else
                        dt = 0;
                    end
                    ...
                    phi_dcm(i,1)    = -asin(uint_g(2));
                    theta_dcm(i,1)  = atan2(uint_g(1),uint_g(3));
                    
                    disp(['time: ',num2str(time(i,1)),', ' ...
                          'yaw: ', num2str(rad2deg(atan2(uint_g(2),uint_g(1))))])
                    if i<=100
                        time_plot  = time;
                        phi_plot   = roll;
                        theta_plot = pitch;
